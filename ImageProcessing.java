...
public class ImageProcessing {
    ...
    //viewImageData(imageData);
		...
		twoDToImage(trimmed, "./trimmednn_apple.jpg");

    int[][] negative = negativeColor(imageData);
    twoDToImage(negative,"./negativecolor_apple.jpg");

    int[][] shrikImage = stretchHorizontally(imageData);
    twoDToImage(shrikImage,"./shrik_apple.jpg");

    int[][] shrickV = shrinkVertically(imageData);
    twoDToImage(shrickV,"./vertical_apple.jpg");

    int[][] inVert = invertImage(imageData);
    twoDToImage(inVert,"./invert_apple.jpg");

    int[][] colorFiller = colorFilter(imageData,30,124,222);
    twoDToImage(colorFiller,"./color_apple.jpg");
    
    int[][] blankImg = new int[500][500];
    int[][] randomColor = paintRandomImage(blankImg);
    twoDToImage(randomColor,"./random_apple.jpg");

    int[] rgba = {255,255,0,255};
    
    int[][] painTing = paintRectangle(blankImg,230,230,100,100,getColorIntValFromRGBA(rgba));
    twoDToImage(painTing,"./painting_apple.jpg");

    int[][] newCanvas = generateRectangles(blankImg,100);
    twoDToImage(newCanvas,"./new_apple.jpg");
    ...
    for(int i = 0; i<imageTwoD.length;i++){
      for(int j = 0; j<imageTwoD[i].length;j++){
        k = j*2;
        manipulatedImg[i][k] = imageTwoD[i][j];
        manipulatedImg[i][k+1] = imageTwoD[i][j];
	...
	public static int[][] shrinkVertically(int[][] imageTwoD) {
		// TODO: Fill in the code for this method
    int[][] manipulatedImg = new int[imageTwoD[0].length/2][imageTwoD.length];
    ...
    for(int i = 0; i<imageTwoD.length;i++){
      for(int j = 0; j<imageTwoD[i].length;j++){
        manipulatedImg[i][j] = imageTwoD[(imageTwoD.length - 1)-i][(imageTwoD[0].length-1)-j];
      }
    }
		return manipulatedImg;
	}
    ...
    for(int i =0; i < numRectangles;i++){
        ...
        int randomColor = getColorIntValFromRGBA(color);
